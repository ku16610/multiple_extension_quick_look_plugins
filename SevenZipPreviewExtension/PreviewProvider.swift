...
class PreviewProvider: QLPreviewProvider, QLPreviewingController {
    ...
    func providePreview(for request: QLFilePreviewRequest, completionHandler: @escaping (QLPreviewReply?, Error?) -> Void) {
        ...
        guard let data = html.data(using: .utf8) else {
            completionHandler(nil, NSError(domain: "SevenZipPreviewError", code: 2,
    ...
    private struct SevenZipEntry {
        let path: String
        let size: Int64
        let isDirectory: Bool
    }

    private enum SevenZipError: Error {
        case cannotOpen
        case parseFailed(description: String)
    }

    private func listEntries(from url: URL) throws -> [SevenZipEntry] {
        guard let jsonStr = archive_list_entries(url.path) else {
            throw SevenZipError.cannotOpen
        }
        defer { archive_free_json(jsonStr) }

        let str = String(cString: jsonStr)
        guard let data = str.data(using: .utf8) else {
            throw SevenZipError.parseFailed(description: "invalid encoding")
        }

        let json = try JSONSerialization.jsonObject(with: data) as? [[String: Any]]
        guard let items = json else {
            throw SevenZipError.parseFailed(description: "invalid JSON")
        }

        return items.compactMap { item -> SevenZipEntry? in
            guard let name = item["name"] as? String,
                  let size = item["size"] as? Int64,
                  let isDir = item["isDirectory"] as? Bool
            else { return nil }
            return SevenZipEntry(path: name, size: size, isDirectory: isDir)
        }
    }

    // MARK: - HTML Generation

    private func generateHTML(entries: [SevenZipEntry], filename: String) -> String {
